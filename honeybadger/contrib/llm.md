...
`LLMHoneybadger(instruments=None, tracer_provider=None, export="events")` is the constructor: `instruments` restricts/selects which provider instrumentors to activate (default: auto-detect all known providers whose SDK is importable — currently `"openai"` and `"anthropic"`; `"bedrock"` is registered but explicit-only and never auto-detected, see "Bedrock: explicit-only registration + the OTLP GenAI gate" below); `tracer_provider` is the borrowed-provider escape hatch; `export` is `"events"` (default, `llm.*` Insights events) or `"otlp"` (scrubbed spans to Honeybadger's OTel endpoint, requires the separately-installed `opentelemetry-exporter-otlp-proto-http` package).
