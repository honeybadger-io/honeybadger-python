...
- **Exclusion-before-insert.** The `exclude_models` check (`_excluded()`) runs before the dedup-LRU insert (`_export_one`, `honeybadger/contrib/llm/_bridge.py:96-110`), so an excluded event can never suppress its twin from being excluded too — an event that should never be emitted doesn't get to "consume" the dedup slot on behalf of one that should.
