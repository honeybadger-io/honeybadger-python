...
def versions_match():
    ...
    for django_version, supported in VERSION_MATRIX.items():
        if (
            importlib.metadata.version("django").startswith(django_version)
            and supported
        ):
            return True
    return False
...
class DjangoPluginTestCase(unittest.TestCase):
    ...
    def test_generate_payload_get(self):
        ...
        payload = self.plugin.generate_payload(
            self.default_payload, self.config, {"foo": "bar"}
        )
...
class DjangoMiddlewareIntegrationTestCase(SimpleTestCase):
    ...
    @unittest.skipUnless(
        versions_match(),
        "Current Python version unsupported by current version of Django",
    )
    def test_context_cleared_after_response(self):
        self.assertIsNone(
            current_request(), msg="Current request should be empty prior to request"
        )
    ...
    @override_settings(
        MIDDLEWARE=[
            "honeybadger.contrib.django.DjangoHoneybadgerMiddleware",
    ...
    def test_exceptions_handled_by_middleware_with_custom_middleware(self):
        def assert_payload(req):
            ...
            self.assertEqual(
                error_payload["error"]["message"], "Custom Middleware Exception"
            )
