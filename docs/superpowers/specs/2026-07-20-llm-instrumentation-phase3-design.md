...
6. **Agents SDK native trace exporter — explicit decision.** Default: **leave it active**. Rationale: the export is the Agents SDK's own default, running before and without Honeybadger — anyone using the SDK already has this data flow. Disabling it as a side effect of installing our instrumentation would change a pre-existing behavior of the user's app (the same "never disturb another telemetry consumer" principle behind skipping already-instrumented SDKs), and its failure mode is nasty action-at-a-distance ("my OpenAI traces dashboard went empty"). Note this is NOT about protecting an installed base of ours — we have none for this integration; it's about not subtracting someone else's feature. Consequence documented prominently: with the Agents SDK, prompts/tool data continue flowing to OpenAI's own trace ingestion regardless of Honeybadger's content flags. Escape hatch: `LLMHoneybadger(instrument_options={"openai_agents": {"disable_openai_trace_export": True}})` — a new constructor arg, a dict of per-registry-key kwargs passed through to `instrumentor.instrument(**kwargs)`. Generic (any instrumentor option), no per-option API surface.
